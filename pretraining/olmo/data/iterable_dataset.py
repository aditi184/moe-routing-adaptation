...
class IterableDataset(torch.utils.data.IterableDataset[Dict[str, Any]]):
    ...
    def _build_and_save_global_indices(self):
        ...
        # Only a single global writer should build the indices file on shared filesystems.
        # Using fs_local_rank here can result in one writer per node (and thus races)
        # when all nodes share the same work_dir, which causes mmap size errors.
        if self.rank == 0:
