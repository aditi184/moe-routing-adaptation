    ...
    def restore_checkpoint(
        ...
        *,
        ...
        if checkpoint_type is None:
            load_path_str = str(load_path).rstrip("/")
            # Check if path ends with "-unsharded"
            if load_path_str.endswith("-unsharded"):
                checkpoint_type = CheckpointType.unsharded
                log.info(f"Detected unsharded checkpoint (path ends with '-unsharded'): {load_path}")
            else:
                # Check if checkpoint contains model.pt (unsharded format) or model/metadata.json (sharded format)
                try:
                    checkpoint_path = Path(load_path).resolve()
                    if (checkpoint_path / "model.pt").exists():
                        checkpoint_type = CheckpointType.unsharded
                        log.info(f"Detected unsharded checkpoint (found model.pt): {load_path}")
                    elif (checkpoint_path / "model" / "metadata.json").exists():
                        checkpoint_type = CheckpointType.sharded
                        log.info(f"Detected sharded checkpoint (found model/metadata.json): {load_path}")
                    else:
                        # Default to sharded if we can't determine (will fail with clear error if wrong)
                        checkpoint_type = CheckpointType.sharded
                        log.warning(
                            f"Could not auto-detect checkpoint type for {load_path}. "
                            f"Defaulting to sharded. If this is incorrect, specify checkpoint_type explicitly."
                        )
                except Exception as e:
                    log.warning(
                        f"Error detecting checkpoint type for {load_path}: {e}. "
                        f"Defaulting to sharded."
                    )
                    checkpoint_type = CheckpointType.sharded
        ...
        elif checkpoint_type == CheckpointType.sharded:
